...
ssh "${CLUSTER_HOST}" "bash -s" << 'EOF'
  ...
  cd "experiments/encoders/XLM-RoBERTa base/FLORES_full_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting FLORES Table 1 (100 sents)..."
  cd "experiments/encoders/XLM-RoBERTa base/FLORES_table1_100_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork
  ...
  cd "experiments/encoders/XLM-RoBERTa base/bible_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting Bible Table 1..."
  cd "experiments/encoders/XLM-RoBERTa base/bible_table1_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork
