...
ssh "${CLUSTER_HOST}" "bash -s" << 'EOF'
  ...
  cd "experiments/qwen/qwen3 8B/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting Qwen3.5 9B — FLORES Table 1 (2000 sents)..."
  cd "experiments/qwen/qwen3.5 9B/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting Apertus 8B — FLORES Table 1 (2000 sents)..."
  cd "experiments/apertus/apertus 8B/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting XLM-RoBERTa large — FLORES Table 1 (2000 sents)..."
  cd "experiments/encoders/XLM-RoBERTa large/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting XLM-RoBERTa base — FLORES Table 1 (2000 sents)..."
  cd "experiments/encoders/XLM-RoBERTa base/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork

  echo "Submitting LaBSE — FLORES Table 1 (2000 sents)..."
  cd "experiments/encoders/LaBSE/FLORES_table1_2000_experiment" && sbatch run_coma_cluster.slurm && cd ~/MEXA-fork
