"""Compute Basque-pivot MEXA scores (standard AND centered) for all models.

Basque (eus_Latn) is a language isolate — a maximally peripheral pivot that
should show the strongest hubness/geometry effects in the cross-pivot
comparison.

Run on the cluster from the repo root:
    python3 scratch/compute_basque_pivot.py
"""
import pickle
import numpy as np
import os
...
for model_dir, model_name, suffix in MODELS:
    ...
    for centered in (False, True):
        d = os.path.join(model_dir, "FLORES_table1_100_basque_pivot_scores", "centered" if centered else "standard")
    ...
    for centered in (False, True):
        ...
        out_csv = f"dashboard/public/data/flores_table1_100_{suffix}_basque_pivot_{seg}results.csv"
        subprocess.run([
