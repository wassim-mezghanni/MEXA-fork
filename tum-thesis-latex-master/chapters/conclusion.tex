...
This thesis investigated the internal cross-lingual representation space of modern autoregressive large language models, evaluating whether training-free representation alignment can reliably assess multilingual capabilities across hundreds of languages. Using the MEXA framework across diverse model families, architectures, and parallel corpora, this work provided empirical insights into parameter scaling, sparse routing dynamics, representation geometry, and tokenization effects.

\section{Summary}
\label{sec:conclusion-summary}

The primary contributions and empirical findings of this thesis are summarized as follows:

\paragraph{Parameter scaling and domain robustness.}
Across new open-weight decoder families (including Qwen~3, Qwen~3.5, and Apertus), cross-lingual alignment scales consistently with model capacity. Scaling within the Qwen~3 family from 0.6B to 8B parameters yields steady gains in peak alignment on FLORES-200 ($\mu_{\text{Max}} = 0.35 \rightarrow 0.57$). Furthermore, language difficulty rankings remain stable across distinct parallel domains ($\rho = 0.94$ between FLORES-200 news translation and the 1,401-language sPBC Bible corpus), demonstrating that internal alignment reflects underlying linguistic competence rather than domain-specific memorization.

\paragraph{Architectural paradigms and layerwise trajectories.}
Comparing causal decoders to dedicated sentence encoders and masked language models revealed distinct representational trajectories across depth. Dedicated sentence encoders (LaBSE, mE5) achieve high alignment early in the network ($\mu_{\text{Max}} > 0.85$ by Layer 6--8) due to explicit contrastive pair objectives. In contrast, causal decoders construct a language-agnostic semantic space progressively, reaching peak alignment in upper-middle layers ($l/L \approx 0.6$--$0.8$) before diverging at final output heads to accommodate language-specific vocabulary projection.

\paragraph{Mixture-of-Experts and routing dynamics.}
Sparse Mixture-of-Experts models (Mixtral~8x7B, Mixtral~8x22B, and Gemma~4~26B-A4B) match or exceed dense baselines with comparable per-token active compute. Analysis of token-level gating showed that routing operates on syntactic and semantic roles rather than language identity. Gating distributions converge to near-uniform expert usage ($\approx 12.5\%$) across diverse language families in intermediate layers, confirming that sparse conditional execution preserves a shared multilingual representation manifold.
...
Subword tokenization heavily penalizes low-resource and non-Latin scripts. Analysis revealed a significant negative log-linear relationship between tokenizer fertility and alignment scores ($r = -0.56, \rho = -0.67$). Within individual script families, higher fertility strongly correlates with reduced alignment (e.g., $r = -0.93$ for Cyrillic, $r = -0.88$ for Arabic). Fragmented subword sequences dilute positional pooling weights and introduce noise into sentence embeddings, creating an artificial performance barrier independent of core model reasoning capacity.
...
\begin{enumerate}
    \item \textbf{Parallel corpus dependencies and domain shifts.} The evaluation relies on sentence-level translation quality. While FLORES-200 provides professionally translated text, the massively multilingual Bible corpus contains archaic phrasing, stylized syntax, and culturally specific references that may not fully reflect contemporary vernacular language usage.
    ...
    \item \textbf{Computational demands of layerwise extraction.} Extracting and storing full hidden state tensors across dozens of layers for 70B+ parameter models over thousands of sentences requires significant GPU memory and disk I/O throughput, constraining continuous evaluation across arbitrary checkpoints during training.
    
    \item \textbf{Absence of generation-specific dynamics.} While MEXA correlates strongly with zero-shot classification and multiple-choice reasoning benchmarks, it does not directly evaluate autoregressive generative behavior, hallucination tendencies, or safety alignment during free-form text generation.
\end{enumerate}
