...
| gpu-nvidia | 2x Intel Xeon Gold 6438Y+<br>Sapphire Rapids (32C/64T) | 1 TB | 880 GB NVMe SSD | 2x NVIDIA L40S (48 GB)<br>`--gres gpu:nvidia:1` | develop |
...
| gpu-intel  | 1x AMD Ryzen 9 7900X3D (12C/24T) | 128 GB | 916 GB NVMe SSD | 2x Intel A770 (16 GB) | develop |
| rocinante | 2x AMD EPYC 7713<br>Milan (64C/128T) | 512 GB | 880 GB NVMe SSD | 1x NVIDIA A100 (40 GB)<br>2x NVIDIA A2 (16 GB)<br>1x NVIDIA P100 (16 GB)<br>3x AMD MI50 (16 GB) <br>1x Intel A770 (16 GB) | develop |

## No GRES on rocinante and gpu-intel
We do not set up GRES for rocinante and gpu-intel nodes due to difficulty of mixed-architectures and undeterminstic id assignment.

When get the resource on these nodes, you will obtain access to all gpus. Thus, not using it for the performance-critical report, or get the entire node for performance measurement.

For Intel gpu, use `ONEAPI_DEVICE_SELECTOR` to select the gpus.  
For NVIDIA gpu, use `CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=<id>` to select the gpus. Note. suggest using PCI_BUS_ID as order to get consistent behavior.  
For AMD gpu, use `HIP_VISIBLE_DEVICES=<id>` to select the gpus.
