...
\begin{itemize}
    ...
    parameters a family trained with a  more multilingual
    data mixture than Llama, in this family the size sweep isolates the
    effect of scale 
...
\begin{itemize}
    ...
    variant extended to 500+ languages, relevant for the
    ...
    0.6B, 4B, and 8B, which share the Qwen3 architecture but are fine-tuned
...
\begin{itemize}
    \item \textbf{FLORES benchmark subset:} the 116 languages overlapping
    with Belebele, with $n = 100$ parallel sentences per language
    the exact benchmark configuration in \citet{kargaran2025mexa}, used
    for reproduction and for all model-family comparisons;
    \item \textbf{FLORES (1012):} the same 116 languages with all
    $n = 1{,}012$ devtest sentences, to quantify the effect of sample
    size on the retrieval criterion (larger $n$ means more distractors
    and therefore a strictly harder task);
    \item \textbf{FLORES Full:} all 203 language script pairs, for
...
\begin{itemize}
    \item \textbf{Bible benchmark subset:} 101 languages with $n = 103$
    parallel sentences , matching the original paper's Bible configuration;
...
configuration; last-token embeddings and the centered variant are reported as ablations. The analysis is  on four levels:
...
\begin{itemize}
    ...
    languages both cover, testing whether alignment is enhacing
    task performance; cross-model Pearson correlations of per-language score profiles  measure how consistent alignment
